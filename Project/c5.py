...
def algorithm(z, hessian, rhv, G, A, C, g, b, d, n, m, p, N, verbose=0):
    epsilon = 1e-16
    lbd = z[-2 * m:-m]
    s = z[-m:]
    # predictor substep
...
def c5(data_folder_path, n, verbose=0):
#     n = 100 - for problem 1
#     n = 1000 - for problem 2
    m = 2 * n
    p = n // 2
    N = n + p + 2 * m
    ...
    while loop and iloop < niter:
        loop, z, KKT, rhv = algorithm(z, KKT, rhv, G, A, C, g, b, d, n, m, p, N, verbose)
        eq, ineq = constraint(z, A, C, b, d, n)
        if verbose:
            print("i={} f = {} eq:{} ineq:{} x_norm:{}".format(
            iloop, f(z, G, g, n), eq, ineq, np.linalg.norm(z[:n])))
        iloop = iloop + 1
