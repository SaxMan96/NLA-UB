    ...
    for i in range(n):
        if i == i_to_pass:
            ...
            if i == n - 1 or abs(D[i, i + 1]) < 1e-16:
                ...
                xi = (b[i] * D[i + 1, i + 1] - b[i + 1] * D[i, i + 1]) / (
                    D[i, i] * D[i + 1, i + 1] - D[i + 1, i] * D[i, i + 1]
                )
                x[i] = xi
...
def algorithm(z, hessian, rhv, G, A, C, g, b, d, n, m, p, N, verbose):
    epsilon = 1e-16
    lbd = z[-2 * m : -m]
    s = z[-m:]
    # predictor substep
...
def c6(data_folder_path, n, verbose=0):
#     n = 100
    m = 2 * n
    p = n // 2
    N = n + p + 2 * m
    A = read_matrix(data_folder_path + "/A.dad", (n, p))
    G = read_matrix(data_folder_path + "/G.dad", (n, n), True)
    C = read_matrix(data_folder_path + "/C.dad", (n, m))
    b = read_vector(data_folder_path + "/b.dad", p)
    d = read_vector(data_folder_path + "/d.dad", m)
    g = read_vector(data_folder_path + "/g_small.dad", n)
    # g = np.random.rand(n)
    z = np.zeros(N)
    z[n:] = 1
    ...
    while loop and iloop < niter:
        loop, z, small_KKT, rhv = algorithm(
            z, small_KKT, rhv, G, A, C, g, b, d, n, m, p, N, verbose
        )
        eq, ineq = constraint(z, A, C, b, d, n)
        if verbose:
            print(
                "i={} f = {} eq:{} ineq:{} x_norm:{} loop:{}".format(
                    iloop, f(z, G, g, n), eq, ineq, np.linalg.norm(z[:n]), loop
                )
            )
        iloop = iloop + 1
    if verbose:
